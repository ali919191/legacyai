...
## Project Biography (How the System Became Operational)

Legacy AI started as a cognitive architecture blueprint and has been progressively converted into executable service code. Instead of building one monolithic model first, the project evolved as composable reasoning layers:

1. **Memory capture and retrieval foundation**
- `backend/app/services/memory_capture_service.py`
- `backend/app/services/memory/memory_embedding_service.py`
- `backend/app/services/memory/vector_store.py`

2. **Narrative and structure layer**
- `backend/app/services/timeline_engine.py`
- `backend/app/services/episode/episode_service.py`
- `backend/app/services/ai/life_story_generator.py`

3. **Reasoning quality layer**
- `backend/app/services/ai/memory_priority_service.py`
- `backend/app/services/ai/memory_grounding_service.py`
- `backend/app/services/ai/memory_distillation_service.py`
- `backend/app/services/ai/wisdom_engine.py`

4. **Personalization and safety layer**
- `backend/app/services/entity/person_profile_service.py`
- `backend/app/services/entity/relationship_service.py`
- `backend/app/services/ai/recipient_context_service.py`
- `backend/app/services/security/legacy_access_service.py`
- `backend/app/services/security/response_moderation_service.py`

5. **Delivery layer (API and app wiring)**
- `backend/app/services/ai/conversation_engine.py`
- `backend/app/api/family_interaction_api.py`
- `backend/app/main.py`

This staged progression matters because each new layer constrained errors from the previous layer: ranking improved relevance, grounding reduced hallucination risk, wisdom improved guidance quality, and recipient context improved age-appropriateness.

## Log Files and Diagnostics Paths

The project currently uses a small set of concrete runtime/test artifacts:

- `tests.log`
: Primary persistent test log generated by the custom test logger (`backend/tests/utils/test_logger.py`).

- `memory_embeddings.json`
: Default local vector-store artifact used by embedding search in development when `VECTOR_STORE_FILE` is not overridden. The file is created in the process working directory.

- `backend/data/sample_memories.json`
: Canonical seed-memory dataset used by integration tests and demos.

- `backend/tests/__pycache__/`
: Python bytecode cache (non-source artifact) generated during test runs.

Insight: the architecture is intentionally observable through file-based artifacts before introducing external observability stacks. This keeps local debugging simple while core behavior stabilizes.

## Random/Sample Memories: Source, Flow, and Usage

### Source Files

- `backend/data/sample_memories.json`
: Realistic seed dataset (~50 memories) used for integration and demos.

- `backend/tests/test_integration_pipeline.py`
: Contains both in-file fallback fixtures (`SEED_MEMORIES`) and dataset loader logic (`_load_seed_memories`) controlled by `LEGACY_SAMPLE_MEMORIES_FILE`.

### How They Are Used

1. Integration test startup seeds memories into `MemoryCaptureService`.
2. Each seeded memory is embedded through `MemoryEmbeddingService.store_memory_embedding(...)`.
3. Query flow retrieves semantically similar memory IDs.
4. `ConversationEngine` pulls full memory objects, ranks, grounds, and contextualizes them.
5. Optional layers add wisdom/advice, recipient adaptation, and moderation before final response.

### Why This Design Is Useful

- It separates **memory realism** (dataset quality) from **reasoning quality** (service logic).
- It allows deterministic regression tests for retrieval and response behavior.
- It creates a reproducible baseline for future model-provider upgrades.

## Key File Change Map (Relative Paths)

The following files represent the most consequential implementation changes in the current codebase trajectory:

- `backend/app/services/ai/memory_grounding_service.py`
: Adds source validation and strict evidence-bound prompt generation.

- `backend/app/services/ai/wisdom_engine.py`
: Adds lesson extraction, pattern detection, principle synthesis, and advice generation.

- `backend/app/services/ai/recipient_context_service.py`
: Adds recipient profile lifecycle and maturity-level adaptation hooks.

- `backend/app/services/ai/conversation_engine.py`
: Central orchestrator where grounding, wisdom, recipient context, knowledge gaps, ranking, and moderation converge.

- `backend/app/services/ai/memory_distillation_service.py`
: Bridges raw memories to higher-level insights and now integrates wisdom generation.

- `backend/app/main.py`
: Runtime composition root wiring all active services into the API stack.

- `backend/app/api/family_interaction_api.py`
: Public surface for asking questions, retrieving memories/timeline, and handling enhanced question workflows.

- `backend/tests/test_memory_grounding_service.py`
- `backend/tests/test_wisdom_engine.py`
- `backend/tests/test_recipient_context_service.py`
- `backend/tests/test_integration_pipeline.py`
: Verification backbone proving these layers are implemented and interacting correctly.

## Meaningful Insights (Beyond Feature Lists)

- The project is evolving as a **memory reasoning system**, not just a chatbot wrapper. The code repeatedly prioritizes provenance (`grounding`), ranking (`priority`), and user suitability (`recipient context`) before generation.
- Advice quality is treated as an explicit pipeline (`distillation -> wisdom`) rather than a generic prompt trick, which improves maintainability and testability.
- Safety is layered: access control, grounding constraints, recipient adaptation, and moderation each reduce different failure modes.
- The architecture is designed so model upgrades can happen later without rewriting business logic; most behavior is in services, not provider-specific SDK calls.
...
The platform now includes a **MemoryGroundingService** that reduces hallucinations by constraining responses to validated memory evidence.

The service provides:

- `build_context_packet(memories)`: creates a structured evidence packet from retrieved memories
- `validate_memory_sources(memories)`: filters and deduplicates memory sources before prompt construction
- `generate_grounded_prompt(question, memories)`: builds a strict prompt that instructs the model to use only provided memory evidence
...
- review memory priority ranking scores returned by the backend
...
17. **Conversation Engine**: Orchestrates memory retrieval, grounding, context building, recipient adaptation, and response generation
18. **Life Story Generator**: Compiles chronological narratives from memories and episode summaries, creating cohesive life stories with key events and personality evolution
19. **Knowledge Gap Detection**: Identifies unknown people, incomplete memory fields, and missing context during open storytelling
20. **Enhanced Questions Widget**: Displays pending follow-up prompts that users can answer asynchronously
21. **Legacy Access Control**: Implements privacy and access controls for authorized beneficiaries
22. **Response Moderation**: Ensures all AI responses remain appropriate, respectful, and safe for family interactions
23. **AI Response**: Delivers personalized, contextually appropriate answers to user questions
...
The `scripts/run_pipeline_tests.py` script sends **15 diverse end-to-end queries** to the running API and logs every request/response pair. It is the quickest way to validate that the full reasoning pipeline — memory retrieval, conversation engine, grounding, moderation — is wired correctly and returning coherent answers.

### Prerequisites

The API server must be running before you execute the script:
...
```bash
# Default — targets http://localhost:8001
python scripts/run_pipeline_tests.py

# Custom base URL or user ID
python scripts/run_pipeline_tests.py --url http://localhost:8001 --user-id family_01
```
...
| # | Scenario |
|---|----------|
| 1 | Early career lessons |
| 2 | Mistakes and regrets |
| 3 | Parenting advice — discipline |
| 4 | Person lookup (Mike) |
| 5 | Most important life lesson |
| 6 | Family relationships |
| 7 | Lessons from failure |
| 8 | Early life description |
| 9 | Relationship advice |
| 10 | Core values |
| 11 | Career regrets |
| 12 | Advice to younger self |
| 13 | Proudest achievement |
| 14 | Handling adversity |
| 15 | Legacy and impact |

### Output

Progress is printed to the console as each test runs:

```text
------------------------------------------------------------
Legacy AI Platform — Pipeline Test Run
Base URL : http://localhost:8001
User ID  : test_user
Cases    : 15
Started  : 2026-03-16 14:32:10
------------------------------------------------------------

[1/15] Early career lessons
  → OK | In my early career I learned that patience and...
[2/15] Mistakes and regrets
  → OK | One mistake I deeply reflect on...
...
------------------------------------------------------------
Results  : 15 passed, 0 failed out of 15
Log file : /home/user/legacyai/logs/pipeline_test.log
------------------------------------------------------------
```

Full detail is written to `logs/pipeline_test.log` (created automatically), including a **memory trace** for every request:

```text
[2026-03-16 14:32:10]
CASE    : Early career lessons
QUESTION: What did you learn from your early career?
STATUS  : 200 OK
MEMORIES USED:
  * memory_021: "First job experience"
  * memory_034: "Failure in early career"
  * memory_007: "Mentorship from senior colleague"
RESPONSE: I remember my first job experience, working at the downtown office. I recall a failure in early career, struggling with a major project deadline. There was a time when mentorship from senior colleague changed how I approached problems.
------------------------------------------------------------
```

When no memories match the query the block reads:

```text
MEMORIES USED: NONE
```

The console output also shows a per-test memory count:

```text
[1/15] Early career lessons
  -> OK | memories:3 | I remember my first job experience...
```
...
This platform is a **memory-first AI system**: all responses are **exclusively grounded in stored memories** and the LLM/synthesiser is strictly secondary to retrieved evidence. The engine will never fabricate an answer — if the retrieved memories do not contain relevant information, it returns `"I don't remember that clearly."` No external knowledge or hardcoded strings are used.

## Strict Memory Grounding

The response pipeline now uses **strict memory grounding**:

1. Retrieve the top relevant memories using hybrid lexical plus embedding matching.
2. Select only the top 3-5 relevant memories with actual query evidence.
3. Build a memory context block from those selected memories.
4. Inject that memory context and the user question into the response prompt.
5. If no relevant memories are found, return exactly `"I don't remember that clearly."`

There is no longer any fallback to arbitrary memories when retrieval fails. If the system cannot find relevant stored evidence, it does not answer generically.

## Semantic Memory Retrieval

Memory retrieval now uses **real semantic embeddings** instead of placeholder random vectors.

### Embedding model

`MemoryEmbeddingService` uses `sentence-transformers/all-MiniLM-L6-v2` to generate 384-dimensional embeddings for:
...
Each memory is embedded from a canonical text block that combines those fields before storage.

### Retrieval flow

For each user query:

1. Generate a semantic embedding vector for the query.
2. Compute cosine similarity against stored memory embeddings.
3. Retrieve the top-N semantic matches.
4. Combine semantic similarity with keyword evidence using a hybrid score:

```text
final_score = (semantic_similarity * 0.7) + (keyword_score * 0.3)
```

This improves recall quality because the system can now find memories based on **meaning**, not just exact word overlap.

### Why this is better than lexical-only retrieval

- A query like `first job` can now match memories such as `First day as junior engineer` even when the wording differs.
- Emotional and thematic similarity can surface relevant experiences that do not repeat the same keywords.
- The lexical component still helps preserve precision for names, titles, and direct phrase matches.

Every response generated by the Conversation Engine is synthesised from stored memories. No external knowledge is used and no hardcoded strings are returned.

### How the pipeline enforces memory-grounding

1. **Hybrid retrieval** — `ConversationEngine` first scores memories using lexical overlap from the query against titles, descriptions, tags, and people involved, then blends that with embedding similarity from `MemoryEmbeddingService.search_similar_memories()`. Only memories with actual query evidence are eligible for selection.

2. **Priority ranking** — `MemoryPriorityService.rank_memories()` re-orders the retrieved memories by a composite score that accounts for importance weight, emotional weight, and recency. The top-ranked memories are used first.

3. **Grounded prompt construction** — `ConversationEngine._construct_prompt()` builds a fully-formatted prompt that embeds the selected memory records (title, description, timestamp, people, location, emotions, tags). The system instruction explicitly states:

   > *Use ONLY the following memories to answer the user's question. Do not use any knowledge outside of these memories.*

4. **Memory-synthesised response** — `ConversationEngine._generate_ai_response(prompt, context)` reads the top 3 memories (sorted by priority score) directly from the context dict and constructs a first-person narrative. Each memory contributes one or more sentences incorporating its description, location, people involved, and emotions. This method is designed as a **drop-in replacement target**: when a real LLM (OpenAI, Azure OpenAI, local model) is integrated, replace the body of `_generate_ai_response` with an API call that sends `prompt` to the model and returns the completion text.

5. **"I don't remember that clearly" guarantee** — if `relevant_memories` is empty at Step 10, the fallback string is returned immediately without calling the synthesiser.

6. **Memory logging** — every call to `_generate_ai_response` logs the memory IDs that contributed to the response at `INFO` level:

   ```
   INFO conversation_engine: Memory IDs used in response synthesis: ['mem-001', 'mem-002', 'mem-003']
   INFO conversation_engine: Generated response for user=alice using 5 memories: ['mem-001', ...]
   ```

### Memory trace in pipeline_test.log

The `/ask` endpoint returns a `memory_details` field alongside `memories_used`. Each entry carries both the memory ID and its human-readable title:

```json
"memory_details": [
  {"id": "memory_021", "title": "First job experience"},
  {"id": "memory_034", "title": "Failure in early career"}
]
```

`scripts/run_pipeline_tests.py` reads this field and writes a structured **MEMORIES USED** block for every test case in `logs/pipeline_test.log`:
...
This makes it straightforward to audit exactly which memories drove a given response, without having to cross-reference IDs manually.

### Replacing the synthesiser with a real LLM

When you are ready to integrate a language model, the only change needed is in `backend/app/services/ai/conversation_engine.py` inside `_generate_ai_response`:

```python
def _generate_ai_response(self, prompt: str, context: Optional[Dict[str, Any]] = None) -> str:
    # Replace this body with an LLM call, e.g. OpenAI:
    import openai
...
#### logs/
Runtime log output directory (auto-created by scripts).
- **pipeline_test.log**: Detailed per-request log generated by `scripts/run_pipeline_tests.py`. Each entry records the timestamp, question, HTTP status, and full response body.

#### scripts/
Utility and operational scripts.
- **README.md**: Explains the purpose of utility scripts for maintenance, data migration, etc.
- **load_memories.py**: Bulk-loads memories from `backend/data/sample_memories.json` into the MemoryCaptureService.
- **run_pipeline_tests.py**: Bulk pipeline test runner. Sends 15 end-to-end queries to the live API at `http://localhost:8001/api/v1/ask` and logs every request/response to `logs/pipeline_test.log`.
