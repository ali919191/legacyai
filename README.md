...
1. **Semantic retrieval** — `MemoryEmbeddingService.search_similar_memories()` returns the top-5 most similar memory IDs for the user query using cosine similarity over stored embeddings. When the vector store is empty (e.g. after a fresh restart before seeding) the engine falls back to the five most-recently added memories so the pipeline never returns an empty set when memories exist.
